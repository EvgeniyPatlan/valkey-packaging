...
`Source7` (`gen-module-sbom.sh`, executed in `%install` to generate the
embedded SBOM -- see "SBOM" below) is the one exception to "supplied from
this directory": it is the repo-root SBOM generator every other product
here uses, one level up from `valkey-admin/`, not this product's own file.
`rpm/build-rpm.sh` and `build-deb.sh` each mount that repo-root `scripts/`
directory into the build container separately (`/repo-scripts`, read-only)
from the `/packaging` mount of this directory, and supply it into `SOURCES`
/ reach it by absolute path the same way as everything else on their
respective sides.
...
Both packages embed a Software Bill of Materials under
`/usr/share/percona-valkey-admin-server/sbom/`: six files (SPDX and
CycloneDX, each as JSON and one other text form, plus a Syft table and a
condensed license manifest) generated by the repo-root
`scripts/gen-module-sbom.sh`, the same generator and file set every other
product in this repo ships (`bloom/`, `search/`, `json/`).

Two things are specific to this package, both because it is the only
npm-based product here:

- The scan target is the **assembled payload**
  (`%{buildroot}%{appdir}` on the RPM side; the copied-in payload tree on
  the DEB side), not the extracted upstream source. The upstream tree
  carries devDependencies (the Vite/Electron toolchain, test frameworks)
  that `common/build-server-payload.sh` deliberately never copies into
  what ships; scanning the source instead of the payload would catalog
  those as if they shipped.
- The invocation sets `SYFT_SELECT_CATALOGERS=+javascript-package-cataloger`.
  Syft's default cataloger set for a directory scan relies on
  `javascript-lock-cataloger` (reads `package-lock.json`/`yarn.lock`), and
  the shipped payload carries no lockfile -- only the runtime
  `node_modules` subset. Without this, the scan silently discovers zero
  packages and all six files ship with empty content -- a real failure
  mode confirmed directly against a built payload, not a hypothetical one.

The generator is not part of the upstream archive (`Source7` on the RPM
side; mounted separately from `/packaging` on the DEB side -- see "Where
the source comes from" above) and requires `syft` on `PATH`, installed and
version-pinned in every `docker/Dockerfile.*`; it fails loudly rather than
emitting a placeholder if `syft` is missing.
