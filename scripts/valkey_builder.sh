...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      RHEL=$(rpm --eval %rhel)
        ...
        ARCH=$(uname -m)
        export ARCH
        INSTALL_LIST="libhiredis-dev libjemalloc-dev liblua5.1-dev liblzf-dev libssl-dev lua-bitop-dev lua-cjson-dev pkgconf tcl tcl-tls pkg-config libsystemd-dev build-essential debconf debhelper devscripts dh-exec git wget build-essential fakeroot devscripts curl make gcc dh-python"
	apt-get update
